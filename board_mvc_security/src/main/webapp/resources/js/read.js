...
$(function() {
	...
	//beforeSend : ajax에 추가하여 header 값으로 보내야 하는 값들을 전송(ajax 코드 안에 해당 코드가 존재해야 함)
	//ajaxSend() : ajax 호출되면 무조건 이 값을 해더로 전송
	...
	//로그인 여부 확인	
	if(!replyer){
		alert('로그인을 한 후 삭제가 가능합니다.');
		modal.modal("hide");
		return;
	}	
	//로그인 사용자 == 댓글 작성자와 같은지 확인
